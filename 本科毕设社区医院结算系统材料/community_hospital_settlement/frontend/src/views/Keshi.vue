    ...
    <el-card shadow="never">
      <el-form :inline="true" :model="query" @submit.prevent>
        <el-form-item label="科室">
          <el-input v-model="query.keshi" placeholder="科室名称" clearable @keyup.enter="load" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table :data="list" v-loading="loading" border stripe>
        <el-table-column prop="keshi" label="科室名称" min-width="200" />
...
const query = reactive({ keshi: '' })
const dialogVisible = ref(false)
const isView = ref(false)
const emptyForm = () => ({ id: null, keshi: '' })
...
async function load() {
  loading.value = true
  try {
    const res = await request.get(`/${userStore.prefix}/keshi`, { params: query })
    list.value = res.data
  } finally {
    loading.value = false
  }
...
async function save() {
  ...
  const base = `/${userStore.prefix}/keshi`
...
async function remove(row) {
  ...
  await request.delete(`/${userStore.prefix}/keshi/${row.id}`)
